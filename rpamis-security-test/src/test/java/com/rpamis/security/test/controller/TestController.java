...
public class TestController {
	...
	public void mbpSelectOne() {
		...
		queryWrapper.lambda().eq(TestVersionDO::getId, testVersionDO.getId());
	...
	public void mbpSelectList() {
		...
		QueryWrapper<TestVersionDO> queryWrapper = new QueryWrapper<>();
		queryWrapper.lambda().in(TestVersionDO::getId, idList);
	...
	/**
	 * 防止已经加密的字段重复加密 如数据库已加密，但出库时加密字段实体未采用注解标识，则此时返回的是加密数据
	...
	public void testCacheIsolate() {
		...
		queryWrapper.lambda().eq(TestVersionV2DO::getId, 1);
		...
		queryWrapper2.lambda().eq(TestVersionDO::getId, testVersionDO.getId());
	...
	public void selectNull() {
		QueryWrapper<TestVersionDO> queryWrapper = new QueryWrapper<>();
		queryWrapper.lambda().eq(TestVersionDO::getId, 999);
		// 断言执行selectOne不会抛出NullPointerException
		TestVersionDO testVersionDO = Assertions.assertDoesNotThrow(() -> testVersionMapper.selectOne(queryWrapper),
