...
/**
 * mybatis动态SQL处理插件 用于处理List入参，foreach情况，需要在update或query最早的解析阶段进行对象处理
 * 如果采用ParameterHandler或ResultSetHandler时处理，此时动态SQL已经准备好了，无论再怎么加密对象 SQL中的动态参数依旧还是未加密的
 * 对于动态SQL的处理在
 *
 * @see org.apache.ibatis.mapping.MappedStatement#getBoundSql(Object) 执行阶段早于Executor调用
 * @see org.apache.ibatis.executor.statement.StatementHandler#parameterize(java.sql.Statement)
 * 处理时需要注意mybatis会将参数变为2个，比如int batchInsertWithList(@Param("testVersionDOList")
 * List<TestVersionDO> testVersionDOList);
 * 此时的Param为testVersionDOList，但mybatis处理parameterObject时，还有默认添加param1为key，value为testVersionDOList的变量用于后续处理
 * 且此时param1的对象引用和testVersionDOList为key的对象相同 注意此时对两个对象的引用保持相同，避免insert操作后拷贝id回源实体不成功
 * @see org.apache.ibatis.executor.statement.PreparedStatementHandler.update
 * 这里选择不进行深克隆，直接对对象进行脱敏，在后续的
 * @see MybatisEncryptInterceptor 再进行深克隆，避免对源对象后置操作(save后再操作)等造成影响
 * @author benym
